...
(function () {
    ...
    let operator = {
        ...
        */
        getSum() {
            return parseFloat(this.aVal) + parseFloat(this.bVal);
        },
        wordLongerThan() {
            return (this.aVal.length > parseInt(this.bVal));
        },
        /*
        Usage: getBigger method takes two arguments and returns the bigger value between them.
         */
        getBigger() {
            if (parseFloat(this.aVal) > parseFloat(parseFloat(this.bVal)))
                return parseFloat(this.aVal);
            else
                return parseFloat(this.bVal);
        },
        /*
         ...
         */
        maxOf() {
            return Math.max(parseFloat(this.aVal), parseFloat(this.bVal));
        }
    ...
    Note: Solved used function (method) Borrowing with ‘apply’
     */
    $("#maxOfTwoNumbers").click(function (evt) {
        ...
        if (num1.trim().toString() !== "" && num2.trim().toString() !== "") {
            ...
            const max = operator.maxOf.apply(temp);
    ...
    $("#maxOfThreeNumbers").click(function (evt) {
        ...
        if (num1.trim().toString() !== "" && num2.trim().toString() !== "" && num3.trim().toString() !== "") {
            ...
            let max = operator.maxOf.apply(temp);
             temp = {
                aVal: max,
                bVal: num3
            };
            max = operator.maxOf.apply(temp);
            result.className = "success";
            result.innerText = "The max number is : " + max;
        } else {
            result.className = "failed";
    ...
    describe("operator", function () {
        ...
        context("when 2 and 3 entered: ", function () {
            ...
            it("the sum is 5", function () {
                assert.equal(operator.getSum(), 5);
            });

            it("Error: the sum is NOT 2", function () {
                assert.equal(operator.getSum(), 2);
            });

            // it("the multiplication product is 6", function() {
            //     assert.equal(getSum(testingValues.aVal,testingValues.bVal), 6);
            // }());
        });
